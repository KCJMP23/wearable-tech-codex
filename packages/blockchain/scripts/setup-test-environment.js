...
async function main() {
    const [deployer, affiliate1, affiliate2, tenant1] = await hardhat_1.ethers.getSigners();
    ...
    const rewardsToken = await hardhat_1.ethers.getContractAt("WearableRewardsToken", deployment.contracts.rewardsToken);
    const affiliateAttribution = await hardhat_1.ethers.getContractAt("AffiliateAttribution", deployment.contracts.affiliateAttribution);
    ...
    const minPayoutAmount = hardhat_1.ethers.parseEther("5"); // 5 WRT minimum
    console.log("🏢 Configuring test tenant...");
    const configureTx = await affiliateAttribution.configureTenant(tenantId, tenant1.address, // payout wallet
    defaultCommissionRate, [await rewardsToken.getAddress()], // authorized tokens
    minPayoutAmount);
    await configureTx.wait();
    console.log("✅ Test tenant configured");
    // 2. Register test affiliates
    console.log("\n👥 Registering test affiliates...");
    const registerAff1Tx = await affiliateAttribution.registerAffiliate(affiliate1.address, "affiliate1@test.com", JSON.stringify({
    ...
    const registerAff2Tx = await affiliateAttribution.registerAffiliate(affiliate2.address, "affiliate2@test.com", JSON.stringify({
    ...
    console.log("\n💰 Minting test tokens...");
    const testTokenAmount = hardhat_1.ethers.parseEther("10000"); // 10,000 WRT for testing
    const mintTx = await rewardsToken.mint(tenant1.address, testTokenAmount, "Test token allocation for affiliate payouts");
    await mintTx.wait();
    console.log(`✅ Minted ${hardhat_1.ethers.formatEther(testTokenAmount)} WRT to tenant wallet`);
    ...
    const approveTx = await rewardsTokenAsTenant.approve(await affiliateAttribution.getAddress(), testTokenAmount);
    ...
    for (let i = 0; i < 5; i++) {
        ...
        // Register click
        const clickId = hardhat_1.ethers.keccak256(hardhat_1.ethers.toUtf8Bytes(`${affiliate}-${productId}-${Date.now()}-${i}`));
        const clickTx = await affiliateAttribution.registerClick(clickId, affiliate, tenantId, productId, JSON.stringify({
            timestamp: Date.now(),
        ...
        if (i % 2 === 0) {
            const conversionId = hardhat_1.ethers.keccak256(hardhat_1.ethers.toUtf8Bytes(`conversion-${clickId}-${Date.now()}`));
            const conversionValue = hardhat_1.ethers.parseEther((Math.random() * 500 + 100).toFixed(2)); // $100-$600
            const customCommissionRate = i === 0 ? 1000 : 0; // 10% for first conversion, default for others
            const conversionTx = await affiliateAttribution.registerConversion(conversionId, clickId, conversionValue, customCommissionRate, await rewardsToken.getAddress(), JSON.stringify({
                orderId: `ORDER-${Date.now()}-${i}`,
                customerEmail: `customer${i}@test.com`,
                productPrice: hardhat_1.ethers.formatEther(conversionValue),
            }));
            ...
            const payTx = await affiliateAttribution.payCommission(conversionId, await rewardsToken.getAddress());
            await payTx.wait();
    ...
    const stakeAmount = hardhat_1.ethers.parseEther("1000"); // 1000 WRT each
    for (const affiliate of [affiliate1, affiliate2]) {
        const mintStakeTx = await rewardsToken.mint(affiliate.address, stakeAmount, "Test tokens for staking demonstration");
        await mintStakeTx.wait();
        // Stake some tokens
        const rewardsTokenAsAffiliate = rewardsToken.connect(affiliate);
        const stakeTx = await rewardsTokenAsAffiliate.stake(hardhat_1.ethers.parseEther("500"));
        await stakeTx.wait();
    ...
    for (const [index, affiliate] of [affiliate1, affiliate2].entries()) {
        ...
        console.log(`   💰 WRT Balance: ${hardhat_1.ethers.formatEther(balance)}`);
        console.log(`   🥩 Staked: ${hardhat_1.ethers.formatEther(stakedBalance)}`);
        console.log(`   📊 Total Earned: ${hardhat_1.ethers.formatEther(stats[0])}`);
    ...
    console.log(`   💰 WRT Balance: ${hardhat_1.ethers.formatEther(tenantBalance)}`);
    console.log(`   📊 Commission Rate: ${defaultCommissionRate / 100}%`);
    console.log(`   💸 Min Payout: ${hardhat_1.ethers.formatEther(minPayoutAmount)} WRT`);
    // Contract info
    console.log(`\n📦 Contract Addresses:`);
